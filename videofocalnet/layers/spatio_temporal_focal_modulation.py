...
class SpatioTemporalFocalModulation(keras.Model):
    ...
    def call(self, x):
        ...
        x_temporal = ops.reshape(
            x_temporal, [batch_size * height * width, depth, channel]
        )
        ...
        for l in range(self.focal_level):
            ...
            ctx_all_temporal += (
                ops.transpose(ctx_temporal, [0, 2, 1]) * gates_temporal[:, l : l + 1]
            )
        ...
        ctx_global = self.act(
            ops.mean(ops.mean(ctx, axis=2, keepdims=True), axis=3, keepdims=True)
        )
        ...
        modulator_temporal = ops.reshape(
            modulator_temporal, [batch_size, height, width, depth, channel]
        )
        modulator_temporal = ops.transpose(modulator_temporal, [0, 3, 1, 2, 4])
        modulator_temporal = ops.reshape(
            modulator_temporal, [batch_size * depth, height, width, channel]
        )
